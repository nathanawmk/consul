...
- **T0:** ECS starts the task. The `consul-client` and `mesh-init` containers start:
  ...
  - `mesh-init` registers the service for this task and its sidecar proxy into
    Consul. It runs `consul connect envoy -bootstrap` to generate Envoy’s
    bootstrap JSON file and write it to a shared volume.  At this point,
    `mesh-init` exits.

- **T1:** The `sidecar-proxy` container and conditionally the `health-sync` containers start:
  - The `sidecar-proxy` container starts. It runs Envoy by executing `envoy -c <path-to-bootstrap-json>`.
  - The `health-sync` container syncs health checks from ECS to Consul.
...
Health checks are automatically synced from ECS into Consul.  This syncing
automatically happens for all application containers that are marked as
`essential`, have ECS `healthChecks`, and don't have native Consul health
checks configured. The `mesh-init` container creates a TTL health check for
every container that fits this criteria and the `health-sync` container ensures
